...
class Interfaz_CalculadoraGases():
    ...
    def  __init__(self):
        ...
        self.ventana.geometry("385x480")
        self.ventana.resizable(width=False, height=False)

        #ETIQUETAS DE ORIENTACION (NO SE ASIGNAN)
        etiquetaPi=LabelFrame(self.ventana,text="Presion Inicial (atm)",width = 150, height = 45)
        etiquetaPi.propagate(False)
        
        etiquetaVi=LabelFrame(self.ventana,text="Volumen Inicial (litros)",width = 150, height = 45)
        etiquetaVi.propagate(False)
        
        etiquetaTi=LabelFrame(self.ventana,text="Temperatura Inicial (kelvin)",width = 150, height = 45)
        etiquetaTi.propagate(False)

        etiquetaPf=LabelFrame(self.ventana,text="Presion Final (atm)",width = 150, height = 45)
        etiquetaPf.propagate(False)
        
        etiquetaVf=LabelFrame(self.ventana,text="Volumen Final (litos)",width = 150, height = 45)
        etiquetaVf.propagate(False)
        
        etiquetaTf=LabelFrame(self.ventana,text="Temperatura Final (kelvin)",width = 150, height = 45)
        ...
        etiquetaResultadoFrame=LabelFrame(self.ventana,text="Resultado",width = 320, height = 50)
    ...
    def calcular(self):
        ...
        self.resultado = self.calculadora.calcular(presionI,presionF,tempI,tempF,volI,volF,self.atribConstante,self.aCalcular)
